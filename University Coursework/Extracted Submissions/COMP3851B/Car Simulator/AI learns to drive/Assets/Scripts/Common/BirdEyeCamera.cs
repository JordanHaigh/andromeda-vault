...
public class BirdEyeCamera : MonoBehaviour
{
    ...
    public float maxDistanceFromCam = 1f;
    ...
    void FixedUpdate()
    {
        Vector3 carPosition = targetCar.transform.position;
        var distance = Distance(transform.position.z, transform.position.x, carPosition.z, carPosition.x);

        if (locked && distance > maxDistanceFromCam)
        {
            Vector3 direction = carPosition - transform.position;
            direction.y = 0;
