...
public class PreferentialElection extends Election {
    ...
    private boolean allocatePreferences(Map<String, Integer> preferenceTallies, List<Vote> formalVotes) {
        ...
        do {
            ...
            //TODO: I changed this from validCandidates.size() to formalVotes.size()
            boolean hasDominantCandidate = highestTally.getValue().compareTo(formalVotes.size() / 2 + 1) > 0;
            ...
            if (!hasDominantCandidate && !allTied) {
                ...
                for (Map.Entry<String, Integer> eachTally : preferenceTallies.entrySet()) {
                    keyStrings.add(eachTally.getKey());
                }

                for (String key : keyStrings) {
                    Integer value = preferenceTallies.get(key);

                    if (value == lowestVote) {
                        preferenceTallies.remove(key);
    ...
    public <T extends Election.Vote> Result count(T... votes) {
        ...
        Map<String, Integer> preferenceTalliesClone = new HashMap<>();
        for (Map.Entry<String, Integer> eachEntry : preferenceTallies.entrySet()) {
            preferenceTalliesClone.put(eachEntry.getKey(), eachEntry.getValue());
        }


        boolean hasDominantCandidate = allocatePreferences(preferenceTalliesClone, formalVotes);
        ...
        if (!hasDominantCandidate) {
            ...
            for (Map.Entry<String, Integer> eachEntry : relevantFirstPreferences.entrySet()) {
                if(eachEntry.getKey().equals(highestTally.getKey())){
                    continue;
                }
